import {ContentChildren, Host, Input, OnDestroy, OnInit, Optional, QueryList, SkipSelf} from '@angular/core';
...
export class BaseFormComponent implements OnInit, OnDestroy {
    ...
    @ContentChildren(ErrorComponent) public inputErrors: QueryList<ErrorComponent>;
    ...
    public hasControlErrorMessage(): boolean {
        const errorMessage = this.inputErrors.find(error => {
            return this.control.hasError(error.errorName);
        });
